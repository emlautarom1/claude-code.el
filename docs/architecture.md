...
Spawn, resume, kill and delete change which sessions a project has, so they redraw through `claude-code--refresh-views`, which covers every view scoped to that **root** whether or not it is displayed — that is what keeps a view current when the spawn menu runs from some other buffer. `g`, `TAB` and `G` change one view's own presentation only, and redraw that buffer alone.
