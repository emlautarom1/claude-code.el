...
- **Worktree session** — a session spawned with `--worktree`, running in a git worktree under the project's `.claude/worktrees/`. It is listed under its parent project's view, with the worktree named in the *Worktree* column, and stays one across a resume: Claude drops the worktree on a clean exit, so `claude-code-resume` reads the name back out of the transcript and hands it to `--worktree` (see [storage model](storage-model.md#the-binding--worktree-state)). The worktree is a `project.el` project of its own, so a view opened from inside it can list the session as well, under the same worktree name and agreeing on whether it is running — but only while Claude files the transcript under the worktree. Once Claude refiles it under the parent tree, which it may do mid-session, the parent's view is the only one that lists it (see [the filing cwd](storage-model.md#the-filing-cwd--relocated)).
