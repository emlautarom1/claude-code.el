...
Append-only JSONL, one JSON object per line. This package reads **six** fields, all cached by file modification time in `claude-code--transcript-cache`. All but the [entrypoint](#the-entrypoint-stamp) are extracted by scanning **backward** from the end of the file (the values of interest sit near the tail):
...
- **Recap** — the `{"type":"system","subtype":"away_summary","content":…}` line. The CLI writes one when the user comes back to a session after five minutes away, so it describes where things stood **when the user last stepped away**, not how the session ended; a long conversation carries several and the backward scan returns the only one that is current. `awaySummaryEnabled: false` in `settings.json` stops the CLI writing them, and `/recap` prints one on demand without writing a line, so neither leaves anything else to read. The text is one line capped at 400 characters (181–303 here), and a process's first three recaps get ` (disable recaps in /config)` appended — a pointer to a Claude setting this package does not have, so `claude-code--read-recap` strips it. This is the one field whose key is **not** unique to the line that sets it: `queue-operation` and `local_command` lines carry a top-level `content` too, both of them holding arbitrary text, so the scan settles a match on `subtype` rather than on `content` alone.
...
`.claude/worktrees/<name>` is the one place the flag builds a checkout, and it resolves against the **repository root**: a `--worktree` asked for from inside a worktree produces a sibling, and a `/` in the name is flattened to `+` rather than nesting. So the layout is exactly three directories deep, which is what lets `claude-code--worktree-parent` run it backwards (`claude-code--worktree-path` is the forward direction, and `claude-code--worktree-names` lists a project's checkouts for the create menu).

**A name that already exists is entered, not duplicated** (measured against v2.1.272). The CLI reports `Resuming existing worktree at: …`, warning that it may carry an earlier session's commits. It may instead report `Reset existing worktree to the current base`, which is a `git reset --hard origin/<default>` — but only for a checkout on its own `worktree-<name>` branch, clean, and fully merged upstream, so nothing is lost. A name outside letters, digits, dots, underscores and dashes is a hard error (`Invalid worktree name`), the empty name included.
...
The token after that prefix decides **membership only** — which project's view lists the transcript — and never leaves `claude-code--project-transcripts`, which is what keeps that true. It is not a name: the encoding is lossy, so a dot or `/` in the worktree name reads as a hyphen, and two names differing only in flattened characters share one transcript directory. Nor is it evidence that a session ran in a worktree — a session started *inside* a worktree directory with no `--worktree` files its transcript there while being bound to no worktree at all. Only [the binding](#the-binding--worktree-state) names one.
...
It is a routine, repeating stamp — written into the same metadata block as `last-prompt` and `ai-title`, so a long session carries dozens — and the CLI resolves a transcript's cwd as the newest `relocatedCwd`, falling back to the `cwd` on the head line. **A live worktree session's transcript can therefore already sit in the parent project's directory while the session is still bound to its worktree**, its head line still naming the worktree it opened in. Once that happens the transcript belongs to the parent root **alone**: a query rooted at the worktree finds nothing, while the parent's finds the session and names the worktree from the binding below. Nothing here reads `relocated`; the file's actual location is what `claude-code--project-transcripts` walks, and the binding is what names the worktree.

This is why the package never scopes to a worktree. A worktree-rooted view would list whichever sessions Claude had not refiled yet — a subset that shrinks while the reader watches, and one that can exclude the very session that made the worktree. `claude-code--current-project` hands a buffer inside a worktree the parent root instead, and the create menu asks for the worktree by name.
