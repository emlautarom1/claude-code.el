...
Arguments arrive as JSON: an omitted optional argument, JSON `null` and JSON `false` all reach the handler as nil, so a boolean argument reads as a plain Lisp truth value. A missing *required* argument is a JSON-RPC `-32602`; a handler that signals an error yields an `isError` result carrying the message, which Claude sees and can act on.
