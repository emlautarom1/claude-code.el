...
| `worktree` | `string` | no       | Name of the git worktree to run the instance in        |

The name, model, effort and worktree the [create menu](../README.md#usage) offers a user, plus one the menu does not: `prompt`, since the menu starts every session idle. `name` is the session's own display name, the one the sessions view shows, while `worktree` names the checkout it runs in. What the tool does *not* take is a directory: the root is the **caller's own** `claude-code--mcp-session-cwd`, so an instance always lands in the tree its parent is working in, and a call whose session id names no known session is refused rather than started somewhere arbitrary. A worktree moves the instance out of that tree, carrying none of the caller's uncommitted work. Naming one that already exists puts the instance *in* it rather than building another beside it; the CLI reports that as `Resuming existing worktree`, and the checkout may carry an earlier session's commits. Where the menu also takes the unnamed request — `--worktree`, which has Claude name a fresh checkout — this tool reaches only the named half of `claude-code-spawn`'s `:worktree`.

Nothing here checks the name: it reaches the CLI as given, which makes both the directory and the branch. The tool asks for letters, digits, dots, underscores and dashes, which is what the CLI accepts intact ([the rest](storage-model.md#worktrees) it either rewrites or rejects, and a rejected name is an instance that dies at startup). Neither the model nor the effort is held to a set of values: both are the CLI's own vocabulary, passed through as given, so a level or an alias the CLI gains works here without a change. The display name is not checked either; it is whatever the caller wants read in the view.
