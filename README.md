...
- **Attention notifications** — an instance left waiting raises a desktop notification named after the session; clicking it brings Emacs forward, switching workspace if needed, and pops that instance into view. An instance already on screen in a focused frame stays quiet. Claude raises these on an idle timer — about a minute after a turn ends, [tunable in `~/.claude.json`](docs/architecture.md#attention-notifications). Replaceable with your own handler, or turned off, via `claude-code-notify-function`.
